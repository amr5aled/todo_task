...
import 'package:task/features/todos/providers/todos.dart';
...
void main() {
  ...
  group('Add todo with todosProvider.notifier.add', () {
    test('added todo should be available on the list exposed by todosProvider', () async {
      final container = createContainer(overrides: [
        apiServiceProvider.overrideWith((ref) => createMockedApiClient()),
      ]);
  ...
  group('Update todo with todosProvider.notifier.updateItem', () {
    test('todo should be updated on the list exposed by todosProvider', () async {
      final container = createContainer(overrides: [
        apiServiceProvider.overrideWith((ref) => createMockedApiClient()),
      ]);
      ...
      await container.read(todosProvider.notifier).updateItem(todo);
      await expectLater(
        container.read(todosProvider.future),
    ...
    test('todo should be updated on the value exposed by todoProvider', () async {
      final container = createContainer(overrides: [
        apiServiceProvider.overrideWith((ref) => createMockedApiClient()),
      ]);
  ...
  group('Delete todo with todosProvider.notifier.delete', () {
    test('todo should be gone from the list exposed by todosProvider', () async {
      final container = createContainer(overrides: [
        apiServiceProvider.overrideWith((ref) => createMockedApiClient()),
      ]);
